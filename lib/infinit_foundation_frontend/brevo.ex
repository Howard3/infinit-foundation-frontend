defmodule InfinitFoundationFrontend.Brevo do
  ...
  def registration_email!(to = %To{}, params), do: send_email_with_template!(to, :registration, params)
  def donation_thank_you_email!(to = %To{}, params), do: send_email_with_template!(to, :donation_thank_you, params)
  def weekly_updates_email!(to = %To{}, params), do: send_email_with_template!(to, :weekly_updates, params)
  def semester_updates_email!(to = %To{}, params), do: send_email_with_template!(to, :semester_updates, params)
  def pre_renewal_reminder_email!(to = %To{}, params), do: send_email_with_template!(to, :pre_renewal_reminder, params)
  ...
  defp get_template_id!(template) do
    Application.get_env(:infinit_foundation_frontend, :brevo_templates)[template] || raise("Template #{template} not found")
  end
  ...
  defp get_api_key(), do: Application.get_env(:infinit_foundation_frontend, :brevo_api_key) || raise("BREVO_API_KEY is not set")

  defp get_headers() do
    [{"api-key", get_api_key()}, {"content-type", "application/json"}, {"accept", "application/json"}]
  end

  defp request!(action, data) do
    {path, method} = get_path_and_method(action)
    headers = get_headers()
    {_req, res} = Req.new([method: method, url: @api_host <> path, headers: headers, json: data])
    |> Req.run!()
